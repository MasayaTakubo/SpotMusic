...
public class SpotifyWebPlaybackSDK extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			case "getCurrentTrackDetails":
				try {
					String trackDetailsJson = spotifyService.getCurrentTrackDetails(accessToken);
					response.setContentType("application/json");
					response.setCharacterEncoding("UTF-8");
					response.getWriter().write(trackDetailsJson);
				} catch (Exception e) {
					e.printStackTrace();
					response.setStatus(HttpServletResponse.SC_INTERNAL_SERVER_ERROR);
					response.getWriter().write("{\"error\": \"Failed to fetch track details\"}");
				}
				break;
